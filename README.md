...
If using Windows, we highly recommend using Windows Subsystem for Linux (WSL) as it installation is smoother and there are no performance downsides.
The installation will still work with Windows, but there may be difficulties with some of the packages used for our renderer.
